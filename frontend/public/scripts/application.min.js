 ...
 */
(function(a){if(typeof define==="function"&&define.amd){define(["jquery"],a)}else{a(jQuery)}}(function(e){var a=/\+/g;function d(g){return g}function b(g){return decodeURIComponent(g.replace(a," "))}function f(g){if(g.indexOf('"')===0){g=g.slice(1,-1).replace(/\\"/g,'"').replace(/\\\\/g,"\\")}try{return c.json?JSON.parse(g):g}catch(h){}}var c=e.cookie=function(p,o,u){if(o!==undefined){u=e.extend({},c.defaults,u);if(typeof u.expires==="number"){var q=u.expires,s=u.expires=new Date();s.setDate(s.getDate()+q)}o=c.json?JSON.stringify(o):String(o);return(document.cookie=[c.raw?p:encodeURIComponent(p),"=",c.raw?o:encodeURIComponent(o),u.expires?"; expires="+u.expires.toUTCString():"",u.path?"; path="+u.path:"",u.domain?"; domain="+u.domain:"",u.secure?"; secure":""].join(""))}var g=c.raw?d:b;var r=document.cookie.split("; ");var v=p?undefined:{};for(var n=0,k=r.length;n<k;n++){var m=r[n].split("=");var h=g(m.shift());var j=g(m.join("="));if(p&&p===h){v=f(j);break}if(!p){v[h]=f(j)}}return v};c.defaults={};e.removeCookie=function(h,g){if(e.cookie(h)!==undefined){e.cookie(h,"",e.extend(g,{expires:-1}));return true}return false}}));var useProxy=false;$.support.cors=true;function Request(){if(useProxy){this.apiServer=document.location.protocol+"//"+document.location.host+"/"}else{this.apiServer="http://184.73.160.99:3458"}this.dataType=null;this.server="api";this.async=true;this.cache=true;this.gHeaders=[];this.get=function(c){var b=[];b=this.gHeaders.concat(b);var a=(arguments.length==3&&typeof(arguments[2])==="boolean")?arguments[2]:true;if(this.server==="api"){url=this.apiServer+c}else{if(this.server==="auth"){url=this.authServer+c}else{if(this.server==="data"){url=this.dataServer+c}}}return $.ajax({url:url,timeout:60000,cache:this.cache,async:this.async})};this.post=function(f,e){var c=[];c=this.gHeaders.concat(c);var a=this.dataType;if(typeof arguments[2]!=="undefined"){a=arguments[2]}var b=(arguments.length==3&&typeof(arguments[2])==="boolean")?arguments[2]:true;if(this.server==="api"){url=this.apiServer+f}else{if(this.server==="auth"){url=this.authServer+f}}return $.ajax({url:url,type:"POST",timeout:20000,data:e})};this.put=function(e,c){var b=[];b=this.gHeaders.concat(b);var a=(arguments.length==3&&typeof(arguments[2])==="boolean")?arguments[2]:true;if(this.server==="api"){url=this.apiServer+e}else{if(this.server==="auth"){url=this.authServer+e}}return $.ajax({url:url,type:"PUT",timeout:3000,contentType:"application/json",data:JSON.stringify(c),dataType:this.dataType,success:function(d){return d}})};this.del=function(c){var b=[];b=this.gHeaders.concat(b);var a=(arguments.length==3&&typeof(arguments[2])==="boolean")?arguments[2]:true;if(this.server==="api"){url=this.apiServer+c}else{if(this.server==="auth"){url=this.authServer+c}}return $.ajax({url:url,type:"DELETE",contentType:"application/json",dataType:this.dataType,cache:false,headers:b})};this.generateUUID=function(){var b=[],c="0123456789ABCDEF";for(var a=0;a<36;a++){b[a]=Math.floor(Math.random()*16)}b[14]=4;b[19]=(b[19]&3)|8;for(var a=0;a<36;a++){b[a]=c[b[a]]}b[8]=b[13]=b[18]=b[23]="-";return b.join("")}}var authKey;function main(){authKey=$.cookie("authenticationKey");if(!authKey){doLogin()}else{var b=new Request();var a=b.get("/account/"+authKey);a.success(function(d,c,e){console.log("Profile data:");console.log(d);doLoggedIn()});a.fail(function(c){$.removeCookie("authenticationKey");doLogin()})}}function doLogin(){$(".loginModal").modal("show");$(".action-login").on("click",function(c){c.preventDefault();var a=new Request();var b={email:$(".loginModal .email").val(),password:$(".loginModal .password").val()};var d=a.post("/login",b);d.success(function(f,e,g){$(".loginModal").modal("hide");console.log("Login success");console.log(f);$.cookie("authenticationKey",f.pkg.data.authenticationKey);doLoggedIn()});d.error(function(e){console.log(e);$(".loginModal .alert-block").show().alert()})})}function doLoggedIn(){var a=new Request()}$(function(){main()});